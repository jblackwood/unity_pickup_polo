  ...
  private static void calculateLocationObjective(GameState state, string id)
        {
            ...
            (
                ...
                var goalToDefend = state.team_goalToDefend.Where(kv => kv.Key == state.athlete_teamId[id]).First().Value;
                var locationToBe =
                    athToDefend.transform.position
                    + Quaternion.LookRotation(
                        goalToDefend.transform.position - athToDefend.transform.position
                    ) * Constants.athDefenseOffset;
